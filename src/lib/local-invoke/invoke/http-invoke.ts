...
export default class HttpInvoke extends Invoke {
  ...
  constructor(tunnelService: TunnelService, sessionId: string, creds: ICredentials, region: string, baseDir: string, serviceConfig: ServiceConfig, functionConfig: FunctionConfig, triggerConfig?: TriggerConfig, debugPort?: number, debugIde?: any, tmpDir?: string, debuggerPath?: any, debugArgs?: any, nasBaseDir?: string) {
    super(tunnelService, sessionId, creds, region, baseDir, serviceConfig, functionConfig, triggerConfig, debugPort, debugIde, tmpDir, debuggerPath, debugArgs, nasBaseDir);
    setSigint();
    ...
    process.on('SIGINT', async () => {
      ...
      if (this.tunnelService) { await this.tunnelService.clean(); }
  ...
  async _startRunner(): Promise<void> {
    ...
    const envs = await docker.generateDockerEnvs(this.creds, this.region, this.baseDir, this.serviceName, this.serviceConfig, this.functionName, this.functionConfig, this.debugPort, null, this.nasConfig, this.debugIde, this.debugArgs);
    ...
    this.containerName = docker.generateRamdomContainerName();
    const opts = await dockerOpts.generateLocalStartOpts(proxyContainerName, this.runtime,
      this.containerName,
      this.mounts,
      cmd,
      envs,
    ...
    if (!isCustomContainerRuntime(this.functionConfig?.runtime)) {
      ...
      await promiseRetry(async (retry: any, times: number): Promise<any> => {
        try {
          ...
          const res: any = await this.runner.exec(['curl', `127.0.0.1:${caPort}`], { outputStream: isDebug ? process.stdout : devnull(), errorStream: isDebug ? process.stderr : devnull() });
          if (res === 0) {
          ...
          if (ex.message === 'Function container is closed, exit!') { throw ex; }
  ...
  private async saveInvokeContainerId(): Promise<void> {
    await setKVInState('invokeContainerId', this.runner?.containerId, genStateId(this.creds?.AccountID, this.region, this.serviceName, this.functionName));
  }
          ...
          if (startMutex) {
            ...
            while (startMutex) {
              ...
              if (startMutexOccupiedTime > maxStartMutexOccupiedTime) {
                await this.clean();
                if (this.tunnelService) { await this.tunnelService.clean(); }
                throw new Error(`Restart function container timeout after 300s!Please check if docker runs normally, then runs 'clean' method and try 'setup' again.`);
              }
            }
          }
          stopMutex = true;
